...
- [x] **Panels auto-open on proximity — annoying on mobile.** Walking near a
  ...
  · New `src/ui/InteractPrompt.jsx` — a bottom-centre pill "Read **X**" with an
    `E` key cap on desktop; on `pointer: coarse` it becomes a round action
    button (open-book icon) on the right, above the mode-switch and opposite the
    movement joystick — the text + key cap hide, the icon shows. Window
    `keydown` for `KeyE` opens it on desktop.
